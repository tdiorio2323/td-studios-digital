...
export const CartPreview: React.FC<Props> = ({ open, onClose }) => {
  const { items, totalCount, addItem, decreaseItem, removeItem, clear } =
    useCart();
  ...
  return (
    ...
    >
      ...
      {/* Panel */}
      <aside
        className={`absolute right-0 top-0 h-full w-full sm:w-[380px] bg-black border-l border-white/10 shadow-2xl transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`}
      >
        ...
        <div className="p-4 border-t border-white/10 flex gap-3">
          ...
          <button className="flex-1 px-4 py-2 bg-white/20 border border-white/30 rounded-lg text-white hover:bg-white/30">
            Checkout
          </button>
        </div>
      </aside>
