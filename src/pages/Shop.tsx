...
const fallbackProducts = [...tdSlideProducts, ...shopPageProducts].map(
  (p, idx) => ({
    ...p,
...
export default function Shop() {
  ...
  function appendManual(list: Product[]): Product[] {
    ...
    const appended = [
      ...list,
      ...manualAdds.filter((m) => !existing.has(m.image1)),
    ];
    return appended.map((p, idx) => ({
      ...p,
      description: `number ${idx + 1}`,
    }));
  }
        ...
        <div className="container mx-auto">
          <div className="text-center mb-8 md:mb-10">
            ...
            <p className="text-white/60 mt-2">
              Flip to see details. Tap on mobile.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-10">
          ...
          <p className="text-white/70">
            Luxury strategy creativity — curated products
          </p>
          <div className="mt-4 flex items-center justify-center gap-3">
            ...
            {editMode && (
              <>
                ...
                <button
                  onClick={() => {
                    localStorage.removeItem("shopLayout:v1");
                  }}
                ...
                <button
                  onClick={() =>
                    setSelected(
                      Object.fromEntries(products.map((p) => [p.image1, true]))
                    )
                  }
                  className="px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white hover:bg-white/20"
                >
                ...
                <button
                  onClick={() =>
                    setProducts((prev) =>
                      prev.filter((p) => !selected[p.image1])
                    )
                  }
                  className="px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white hover:bg-white/20"
                >
              ...
              ? products.map((p, i) => (
                  <EditableProductCard
                    ...
                    onChange={(id, patch) =>
                      setProducts((prev) =>
                        prev.map((it) =>
                          it.image1 === id ? { ...it, ...patch } : it
                        )
                      )
                    }
                    onDelete={(id) =>
                      setProducts((prev) =>
                        prev.filter((it) => it.image1 !== id)
                      )
                    }
                    draggable
