...
async function extractHeaderFooter(page) {
  ...
  await page.evaluate(() => {
    ...
    document
      .querySelectorAll(
        '[aria-haspopup="menu"],[data-dropdown-toggle],.dropdown,summary'
      )
      .forEach((el) =>
  ...
  return await page.evaluate(() => {
    ...
    const push = (a, location) => {
      ...
      const dropdown = !!a.closest(
        '[role="menu"],details,[data-dropdown],.dropdown,.menu'
      );
    ...
    // Header area
    document
      .querySelectorAll('header a, header [role="navigation"] a, nav a')
      .forEach((a) => {
    ...
    const headerHTML = headerEl
      ? headerEl.outerHTML.replace(/\s+/g, " ").trim()
      : "";
    ...
    const logo =
      (headerEl &&
        headerEl.querySelector(
          'a[href="/"], a[href="#/"], a:has(img), a[aria-label*="home" i]'
        )) ||
      null;
...
async function mobileMenuProbe(page) {
  ...
  const result = {
    default_closed: null,
    toggles_open: null,
    closes_on_selection: null,
    closes_on_route_change: null,
    body_scroll_locked_when_open: null,
  };
  const headerBtn = await page
  ...
  const navBefore = await page
    .locator('header nav, [role="navigation"]')
    .count();
...
async function routeCheck(context, base, path) {
  ...
  try {
    ...
    const { headerHTML, headerCount, logoHref } =
      await extractHeaderFooter(page);
    ...
    const logo = page
      .locator('header a[href="/"], header a[aria-label*="home" i]')
      .first();
...
async function runForTarget(browser, base) {
  ...
  const deskLinks = deskData.links.map((l) => ({
    ...
    path: (() => {
      try {
        return new URL(l.href, base).origin === new URL(base).origin
          ? normPath(new URL(l.href, base).toString())
          : l.href;
      } catch {
        return l.href;
      }
  ...
  await dp
    .screenshot({
      path: pathJoinSafe(outRoot, "home_desktop.png"),
      fullPage: true,
    })
    .catch(() => {});
  await dp.close();
  ...
  await mp
    .screenshot({
      path: pathJoinSafe(outRoot, "home_mobile_closed.png"),
      fullPage: true,
    })
    .catch(() => {});
  ...
  await mp
    .screenshot({
      path: pathJoinSafe(outRoot, "home_mobile_open.png"),
      fullPage: true,
    })
    .catch(() => {});
  await mp.close();
  ...
  for (const r of all) {
    ...
    const idx = inventory.findIndex(
      (x) =>
        x.location === r.location && x.path === r.path && x.label === r.label
    );
    if (idx >= 0) {
  ...
  // Write outputs
  await fs.writeFile(
    pathJoinSafe(outRoot, "nav-inventory.json"),
    JSON.stringify(inventory, null, 2)
  );
  await fs.writeFile(
    pathJoinSafe(outRoot, "routes-log.json"),
  ...
  for (const r of inventory) {
    md.push(
      `| ${r.label} | ${r.path} | ${r.location} | ${r.order_index} | ${r.desktop ? "yes" : "no"} | ${r.mobile ? "yes" : "no"} | ${r.dropdown ? "yes" : "no"} | ${r.external ? "yes" : "no"} |`
    );
  }
  md.push(`\n## Route Verification`);
  md.push(
  ...
  for (const r of routeResults) {
    md.push(
      `| ${r.route} | ${r.status ?? ""} | ${r.final_url ?? ""} | ${r.header_present ? "yes" : "no"} | ${r.logo_to_home ? "ok" : "no"} | ${r.duplicate_header ? "yes" : "no"} | ${r.console_errors.join("; ").replace(/\|/g, "/")} | ${r.notes?.replace(/\|/g, "/")} |`
    );
  }
  md.push(`\n## Mobile Menu Tests`);
  ...
  md.push(
    `- body_scroll_locked_when_open: ${mobileProbe.body_scroll_locked_when_open}`
  );
