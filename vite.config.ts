...
export default defineConfig(({ mode }) => ({
  ...
  plugins: [
    ...
    // Temporarily disabled custom middleware to fix SPA routing
    false &&
      mode === "development" &&
      // Dev-only utilities
      mode === "development" && {
        ...
        configureServer(server) {
          ...
          server.middlewares.use("/__list-public", (req, res) => {
            ...
            try {
              ...
              const dirs = (
                requested.length ? requested : Array.from(allowed)
          ...
          server.middlewares.use("/__scaffold", (req, res) => {
            ...
            req.on("end", () => {
              try {
                ...
                if (!name || !code) {
                  res.statusCode = 400;
                  res.end("Missing name or code");
                  return;
                }
                const comp = (name as string)
                  .replace(/[^a-zA-Z0-9]+/g, " ")
                  .split(" ")
                  .filter(Boolean)
                ...
                const routePath =
                  route ||
                  `/${comp.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase()}`;
