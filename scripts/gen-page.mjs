...
function upsertPage(name) {
  ...
  if (fs.existsSync(filePath))
    return { componentName, filePath, created: false };
...
function main() {
  ...
  if (!items.length) {
    console.log(
      'Usage: node scripts/gen-page.mjs "Name:/path" "About" --file pages.csv'
    );
    process.exit(0);
  }
  ...
  for (const it of items) {
    ...
    console.log(
      `${created ? "Created" : "Exists "}: src/pages/${componentName}.tsx ${routePath ? `-> ${routePath}` : ""}`
    );
  }
